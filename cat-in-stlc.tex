...
\begin{itemize}
	...
	\item \emph{morphism}: $\dbracket{\Gamma \vdash E:A}:\dbracket{\Gamma} \to \dbracket{A}$, abbreviate it to $\dbracket{E}:\dbracket{\Gamma} \to \dbracket{\Gamma}$;
	\item \emph{identity}: $\dbracket{x:A \vdash x:A} = \ID\left(\dbracket{A}\right):\dbracket{A} \to \dbracket{A}$, it corresponds to \[\infer{x:A \vdash x:A}{}~\textsc{Var}\]
...
\bibitem{b2} Edward Morehouse. Basic Category Theory. OPLSS, 2016.
\end{thebibliography}
\end{document}